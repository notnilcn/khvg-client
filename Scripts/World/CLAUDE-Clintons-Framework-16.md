...
- `CameraController2D.UnregisterCamera()` is called from `LocalPlayer._ExitTree` (alongside the 3D
  follow-target clear). This matters: the registered `PhantomCamera2D` is a child of the `LocalPlayer`
  node, so on death it gets freed and `CameraController2D._localPlayerPCam2D` becomes a dangling
  reference to a disposed node — `_Process` would then throw `ObjectDisposedException` writing
  `RotationOffset`. Both `UnregisterCamera()` and `_Process` guard with `IsInstanceValid` (a plain
  C# `!= null` check does **not** catch a freed Godot node), so the reference is cleared on death and
  the per-frame writer no-ops if it ever races ahead of the unregister.
