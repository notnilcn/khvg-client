...
/// hurtbox (Area2D). Passive — HitZones detect it and report the hit; it emits
...
{
    [Signal] public delegate void DidReceiveDamageEventHandler(HitZone attacker);

    protected override System.Type[] GetRequiredComponents() => new[] { typeof(FactionComponent) };

    public void ProcessHit(HitZone attacker) => EmitSignal(SignalName.DidReceiveDamage, attacker);

    /// Enemy-bullet path: BlastBullets2D reports overlaps through BulletManager rather than
    /// through a HitZone, so bullets reach the same reducer via this method.
